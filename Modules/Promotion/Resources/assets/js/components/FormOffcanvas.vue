    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="form-offcanvas" aria-labelledby="form-offcanvasLabel">
      ...
      <div class="offcanvas-body">
        ...
        <InputField class="col-md-12" type="text" :is-required="true" :label="$t('promotion.lbl_name')"
          placeholder="Enter Name" v-model="name" :error-message="errors['name']"
          ></InputField>
        ...
        <div class="col-md-12" v-if="discount_type">
          ...
          <div v-else-if="discount_type == 'fixed'">
            <InputField type="number" :is-required="true" :label="$t('promotion.discount_amount')" placeholder="Enter Discount Amount"
              v-model="discount_amount" :error-message="errors['discount_amount']"></InputField>
          </div>
        </div>
        ...
        <div>
          <InputField class="col-md-12" type="number" :is-required="true" :label="$t('promotion.use_limit')"
            ...
            :error-messages="errorMessages['use_limit']" :is-read-only="coupon_type == 'bulk' || ISREADONLY"></InputField>
        </div>
...
const validationSchema = yup.object({
  ...
  end_date_time: yup.date().required('end datetime is required')
    .min(new Date(), 'end datetime cannot be in the past').typeError('Enter valid date'),
  ...
  coupon_code: yup.string()
    .when("coupon_type", {
      is: 'custom',
      then: () => yup.string().required('Coupon code is a required field')
  ...
  number_of_coupon: yup.string()
    .when("coupon_type", {
      is: 'bulk',
      then: () => yup.number().min(1, 'Number of coupon must be greater than or equal to 1')
  }),
  discount_amount: yup.string()
      ...
      .when("discount_type", {
        is: "percent",
        then: () => yup.number().typeError('Discount percent must be a number').min(1, 'Discount percent must be greater than or equal to 1')
      })
})
