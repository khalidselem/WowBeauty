    ...
    <div :class="`offcanvas offcanvas-end`" data-bs-scroll="true" tabindex="-1" id="booking-form" aria-labelledby="offcanvasBookingForm">
      ...
      <template v-else-if="SINLGE_STEP == 'CHECK_OUT' && status == 'checkout'">
        ...
        <div class="offcanvas-body border-top">
          ...
          <div v-if="selectedService.length > 0">
            <h5 class="mt-3">Services</h5>
            <ul class="form-group list-group list-group-flush">
              <li v-for="(service, index) in selectedService" :key="index" class="list-group-item py-3 px-1">
                <div class="d-flex flex-column gap-2">
                  <div class="d-flex align-items-center justify-content-between">
                    <h6>{{ service.service_name }} <span :id="service.service_id">({{ formatCurrencyVue(service.service_price) }})</span><span :id="service.service_name" ></span></h6>
...
import { EMPLOYEE_LIST, CUSTOMER_LIST, SERVICE_LIST, SLOT_LIST, PAYMENT_PUT_URL, UPDATE_PAYMENT_DATA, CHECKOUT_URL, STRIPE_PAYMENT_DATA, EDIT_URL, STORE_URL, UPDATE_URL, UPDATE_STATUS,PRODUCT_LIST } from '../constant/booking'
...
var SUB_TOTAL_SERVICE_AMOUNT = computed(() => selectedService.value.reduce((total, service) => total + service.service_price, 0) + selectedProduct.value.reduce((total, product) => total + (product.discounted_price ? product.discounted_price : product.product_price) * product.product_qty, 0));
...
const formSubmitPaynow = () => {
  if(!IS_SUBMITED.value) {
    const values = {payment: payment_data.value}
    IS_SUBMITED.value = true;
