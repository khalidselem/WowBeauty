...
class BookingsController extends Controller
{
    ...
    public function index_data(Datatables $datatable, Request $request)
    {
        ...
        return $datatable->eloquent($query)
            ...
            ->editColumn('service_amount', function ($data) {
                $totalAmount = $data->services->sum(function($service) {
                    return $service->service_price * $service->service_amount;
                });
                return '<span>' . \Currency::format($totalAmount) . '</span>';
            })
    ...
    {
        if (isset($request->action_type) && $request->action_type == 'update-employee') {
            $status = $request->value;
        }

        BookingService::where('booking_id', $id)->update(['employee_id' => $request->value]);
